...
import com.cudo.mproject.Menu.Site.PLN.PLNPhotoActivity;
...
public class ShowPhotoPlnDialog extends Dialog {
...
//    @BindView(R.id.input_layout)
    ...
    PLNPhotoActivity context;

    public ShowPhotoPlnDialog(@NonNull Context context, PLNPhotoActivity plnPhotoActivity, PhotoPLN photoPLN, Realm realm) {
        ...
        this.context = plnPhotoActivity;
        setContentView(R.layout.dialog_photo);
        ButterKnife.bind(this);
        dummy.requestFocus();
        initView(realm, photoPLN);
    }

    void initView(final Realm realm, final PhotoPLN photoPLN) {
    ...
    /*    Glide.with(getContext()).setDefaultRequestOptions(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE))
                ...
                .into(img);*/
            Picasso.with(getContext()).invalidate(FileUtils.getUri(getContext(), photoPLN.getPathPhotoPLN()));
        Picasso.with(getContext()).load(FileUtils.getUri(getContext(),photoPLN.getPathPhotoPLN())).placeholder(R.drawable.ic_photo_camera_black_24dp).into(img);

        if (photoPLN.getStatus_photo().contentEquals("0")) {
...
//            extFile = (String) spinner_extFile.getSelectedItem();
            ...
            desc.setText(photoPLN.getDescription());
//
...
//                    if (!extFile.contentEquals("type")) {
                    PhotoPLN.updateType(realm, photoPLN, desc.getText().toString(), "");
                    dismiss();
//                    }
                }
//                //
