...
public class ShowPhotoIMBDialog extends Dialog {
...
//    @BindView(R.id.input_layout)
    ...
    IMBPhotoActivity context;

    public ShowPhotoIMBDialog(@NonNull Context context, IMBPhotoActivity imbPhotoActivity, PhotoIMB photoIMB, Realm realm) {
    ...
    void initView(final Realm realm, final PhotoIMB photoIMB) {
    ...
    /*    Glide.with(getContext()).setDefaultRequestOptions(new RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE))
                ...
                .into(img);*/
            Picasso.with(getContext()).invalidate(FileUtils.getUri(getContext(), photoIMB.getPathPhotoIMB()));
        Picasso.with(getContext()).load(FileUtils.getUri(getContext(), photoIMB.getPathPhotoIMB())).placeholder(R.drawable.ic_photo_camera_black_24dp).into(img);

        if (photoIMB.getStatus_photo().contentEquals("0")) {
...
//            extFile = (String) spinner_extFile.getSelectedItem();
            ...
            desc.setText(photoIMB.getDescription());
...
//                    if (!extFile.contentEquals("type")) {
                    PhotoIMB.updateType(realm, photoIMB, desc.getText().toString(), "");
                    dismiss();
//                    }
                }
                //
